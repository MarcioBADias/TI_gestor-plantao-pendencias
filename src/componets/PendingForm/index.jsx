...
const PendingForm = ({ selectTech, selectedDate, onClose }) => {
  ...
  const applyFilters = () => {
    return relatorios.filter((relatorio) => {
      ...
      const inicioOk =
        !state.filterDataInicio || createdDate >= state.filterDataInicio
      const fimOk = !state.filterDataFim || createdDate <= state.filterDataFim
      const matchData =
        createdDate === new Date(state.data).toISOString().split('T')[0]
  ...
  const sendMessageOnWhatsapp = async () => {
    ...
    try {
      const response = await fetch(
        'http://192.168.15.65:8080/message/sendText/Plantao',
        {
          method: 'POST',
          headers: {
      ...
      <div style={{ display: 'flex', gap: 10, margin: 20 }}>
        ...
        {state.abrirFormulario ? (
          <FaRegMinusSquare
            ...
            onClick={() => dispatch({ type: 'SET_OPENFORM' })}
          />
        ) : (
          <FaRegPlusSquare
            style={{ width: 40, height: 40, cursor: 'pointer' }}
          ...
          <div key={index}>
            <CardPlantao key={index} className="relatorio-card">
              <div>
                <p>
                  <strong>
                    {formatDate(relatorio.data_atendimento.slice(0, 10))}
                  </strong>
                </p>
                <p>
